( function() {
	...
	const iconPlay = '<svg viewBox="0 0 16 16" aria-hidden="true"><path fill="currentColor" d="M4.2 2.54c0-.78.84-1.27 1.52-.88l6.27 3.63c.68.39.68 1.37 0 1.76L5.72 10.68c-.68.39-1.52-.1-1.52-.88V2.54Z"></path><path fill="none" stroke="currentColor" d="M8 15.25A7.25 7.25 0 1 0 8 .75a7.25 7.25 0 0 0 0 14.5Z"></path></svg>';
	...
	function buildVideoAction( story ) {
		if ( 'video' === story.mediaType && story.videoUrl ) {
			return `
				...
				>
					<span class="goodsleep-story-card__action-icon">${ iconPlay }</span>
	...
	document.addEventListener( 'click', function( event ) {
		const videoButton = event.target.closest( '[data-action="view-video"]' );
		if ( videoButton ) {
			openVideoModal( videoButton.dataset.videoUrl || '' );
			return;
		}
