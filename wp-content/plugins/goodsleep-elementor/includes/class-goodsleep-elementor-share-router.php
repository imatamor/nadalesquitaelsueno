...
class Goodsleep_Elementor_Share_Router {
	...
	public function render_short_route() {
		...
		$story              = $stories[0];
		$media              = goodsleep_get_story_primary_media( $story->ID );
		...
		$page_title         = sprintf( __( '%1$s | %2$s', 'goodsleep-elementor' ), $story_name, get_bloginfo( 'name' ) );
		$download_tooltip   = 'video' === $media['type'] ? __( 'Descargar video', 'goodsleep-elementor' ) : __( 'Descargar audio', 'goodsleep-elementor' );
		...
		echo '<div class="goodsleep-story-single__heading"><p class="goodsleep-story-single__prefix">' . esc_html__( 'A', 'goodsleep-elementor' ) . '</p><h1 class="goodsleep-story-single__title">' . esc_html( $story_name ) . '</h1></div>';
		echo '<p class="goodsleep-story-single__lead">' . esc_html__( 'Nada le quita el sueno', 'goodsleep-elementor' ) . '</p>';
		echo '</div>';
		...
		if ( 'video' === $media['type'] && ! empty( $media['url'] ) ) {
			echo '<button type="button" class="goodsleep-story-card__action-button" data-action="view-video" data-video-url="' . esc_url( $media['url'] ) . '" data-tooltip="' . esc_attr__( 'Ver video', 'goodsleep-elementor' ) . '" aria-label="' . esc_attr__( 'Ver video', 'goodsleep-elementor' ) . '"><span class="goodsleep-story-card__action-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path fill="none" stroke="currentColor" stroke-width="2" d="M3,22.0000002 L21,12 L3,2 L3,22.0000002 Z M5,19 L17.5999998,11.9999999 L5,5 L5,19 Z M7,16 L14.1999999,12 L7,8 L7,16 Z M9,13 L10.8,12 L9,11 L9,13 Z"/></svg></span><span class="goodsleep-story-card__action-label">' . esc_html__( 'Ver video', 'goodsleep-elementor' ) . '</span></button>';
		}
